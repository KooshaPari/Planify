...
def create_project(workspace, user_id):
    ...
    project = Project.objects.create(
        ...
        identifier=name[: _secure_random.randint(2, 12 if len(name) - 1 >= 12 else len(name) - 1)].upper(),
    ...
    _ = ProjectMember.objects.bulk_create(
        [
            ProjectMember(
                ...
                member=member,
                role=20,
                sort_order=_secure_random.randint(0, 65535),
            )
...
def create_cycles(workspace, project, user_id, cycle_count):
    ...
    while len(cycles) <= cycle_count:
        ...
        start_date = start_date_option[_secure_random.randint(0, 1)]
...
def create_modules(workspace, project, user_id, module_count):
    ...
    for _ in range(0, module_count):
        start_date = [None, fake.date_this_year()][_secure_random.randint(0, 1)]
        end_date = (
            None
            if start_date is None
            else fake.date_between_dates(
    ...
    for _ in range(0, pages_count):
        ...
        pages.append(
            Page(
                ...
                workspace=workspace,
                owned_by_id=user_id,
                access=_secure_random.randint(0, 1),
                color=fake.hex_color(),
...
def create_page_labels(workspace, project, user_id, pages_count):
    ...
    bulk_page_labels = []
    for page in pages:
        for label in _secure_random.sample(list(labels), _secure_random.randint(0, len(labels) - 1)):
...
def create_issues(workspace, project, user_id, issue_count):
    ...
    largest_sort_order = Issue.objects.filter(
        project=project, state_id=states[_secure_random.randint(0, len(states) - 1)]
    ...
    for _ in range(0, issue_count):
        ...
        issues.append(
            Issue(
                state_id=states[_secure_random.randint(0, len(states) - 1)],
                project=project,
                workspace=workspace,
                name=text[:254],
                description_html=f"<p>{text}</p>",
                ...
                start_date=start_date,
                target_date=end_date,
                priority=["urgent", "high", "medium", "low", "none"][_secure_random.randint(0, 4)],
                created_by_id=creators[_secure_random.randint(0, len(creators) - 1)],
            )
        )

        largest_sort_order = largest_sort_order + _secure_random.randint(0, 1000)
        last_id = last_id + 1
    ...
    IntakeIssue.objects.bulk_create(
        [
            IntakeIssue(
                issue=issue,
                intake=intake,
                status=(status := [-2, -1, 0, 1, 2][_secure_random.randint(0, 4)]),
                snoozed_till=(datetime.now() + timedelta(days=_secure_random.randint(1, 30)) if status == 0 else None),
...
def create_issue_parent(workspace, project, user_id, issue_count):
    ...
    for sub_issue in sub_issues:
        sub_issue.parent_id = parent_issues[_secure_random.randint(0, int(parent_count - 1))]
...
def create_issue_assignees(workspace, project, user_id, issue_count):
    ...
    for issue in issues:
        for assignee in _secure_random.sample(list(assignees), _secure_random.randint(0, len(assignees) - 1)):
...
def create_issue_labels(workspace, project, user_id, issue_count):
    ...
    bulk_issue_labels = []
    for issue in issues:
        _secure_random.shuffle(shuffled_labels)
        for label in _secure_random.sample(shuffled_labels, _secure_random.randint(0, 5)):
...
def create_cycle_issues(workspace, project, user_id, issue_count):
    ...
    bulk_cycle_issues = []
    for issue in issues:
        cycle = cycles[_secure_random.randint(0, len(cycles) - 1)]
...
def create_module_issues(workspace, project, user_id, issue_count):
    ...
    bulk_module_issues = []
    for issue in issues:
        _secure_random.shuffle(shuffled_modules)
        for module in _secure_random.sample(shuffled_modules, _secure_random.randint(0, 5)):
