...
Subcommands:
    scan           — scan a root for app repos, output drift hits above threshold
    score          — compute drift score for a single candidate repo path
    list-buckets   — list known ADR-023 buckets (PAUSED / CONDITIONAL / CAPSTONE)
                     and which repos fall into each
    validate       — re-print a single drift hit (HITL gate)

Output formats (scan):
    json           — raw DriftHit objects
...
# Bucket labels used in output (must match AGENTS.md §"Active / Paused app-level repos")
BUCKET_LABELS = {
    "paused": "PAUSED",
    "conditional": "CONDITIONAL",
    "capstone": "CAPSTONE (legally mined)",
}
...
# Drift scoring weights (per ADR-049 §3)
W_CAPABILITY = 1.0
W_PORT_MATCH = 0.4
W_ADAPTER_MATCH = 0.3
W_TEST_MATCH = 0.3
...
def detect_buckets(repo_root: Path) -> list[str]:
    """Return which ADR-023 buckets a repo belongs to (may overlap)."""
    name = repo_root.name
    out: list[str] = []
    ...
    for cap_dir, files in by_dir.items():
        ...
        for f in files:
            ...
            if total_bytes >= min_bytes:
                # Read text only once we know the size threshold is plausible
                try:
...
def render_gh_issues(hits: list[DriftHit]) -> str:
    ...
    Each hit becomes its own issue body, separated by `<!-- drift-hit: N -->` markers
    so the consumer (heavy-runner cron → issue auto-create) can split on markers.
    """
    if not hits:
        return "<!-- drift-hit: no hits above threshold -->\n"
    out = []
    for i, h in enumerate(hits):
        out.append(f"<!-- drift-hit: {i} -->")
        out.append(f"## Drift detected: `{h.repo}` ({BUCKET_LABELS.get(h.bucket, h.bucket)})\n")
        ...
        for p in h.candidate_paths:
            out.append(f"- `{p}`")
        out.append("\n### HITL gate\n")
        out.append("A human must confirm this drift hit before the extraction PR is opened. ")
        out.append("Run: `pheno-drift-detector validate --hit <this-file> --yes`\n")
        out.append("\n---\n")
    return "\n".join(out)
...
def render_md(hits: list[DriftHit]) -> str:
    ...
    for h in hits:
        out.append(
            f"### `{h.repo}` ({BUCKET_LABELS.get(h.bucket, h.bucket)}) "
            f"— score {h.drift_score:.2f}\n"
        )
...
def list_buckets(root: Path | None) -> str:
    """List ADR-023 buckets. If root is given, annotate which repos under root fall into each bucket."""
    out: list[str] = ["## ADR-023 App buckets\n"]
    bucket_to_repos: dict[str, list[str]] = {
        "paused": sorted(PAUSED_APPS),
        "conditional": sorted(CONDITIONAL_APPS),
        "capstone": sorted(CAPSTONE_APPS),
    }

    if root is not None and root.is_dir():
        for child in sorted(root.iterdir()):
            if not child.is_dir() or should_skip(child.relative_to(root)):
                continue
            buckets = detect_buckets(child)
            for b in buckets:
                bucket_to_repos.setdefault(b, []).append(child.name)

    for bucket, repos in bucket_to_repos.items():
        label = BUCKET_LABELS.get(bucket, bucket)
        out.append(f"### {label} ({bucket})")
        if not repos:
            out.append("_(none)_\n")
        else:
            for r in repos:
...
def cmd_score(args: argparse.Namespace) -> int:
    """Score a single candidate repo (not requiring the bucket-name match)."""
    repo = Path(args.candidate).resolve()
    if not repo.is_dir():
        print(f"error: candidate is not a directory: {repo}", file=sys.stderr)
        return 1

    caps = find_capability_dirs(repo)
    score, target, rationale = score_drift(caps)
    above = score >= DRIFT_THRESHOLD
    ...
    if args.format == "md":
        # Render a single-hit markdown report
        out = [f"## Drift score: `{repo.name}`\n"]
        out.append(f"**Score:** {score:.2f} (threshold {DRIFT_THRESHOLD})  ")
        out.append(f"**Above threshold:** {'✓ yes' if above else '✗ no'}\n")
        out.append(f"**Target substrate:** `{target}`\n")
    ...
    return 2 if above else 0


def cmd_list_buckets(args: argparse.Namespace) -> int:
    """List the ADR-023 buckets; optionally annotate with repos under --root."""
    root = Path(args.root).resolve() if args.root else None
    sys.stdout.write(list_buckets(root))
    return 0


def cmd_validate(args: argparse.Namespace) -> int:
    """Validate a single hit by re-printing its summary (HITL gate per ADR-049 §5)."""
    path = Path(args.hit).resolve()
    if not path.is_file():
        print(f"error: hit is not a file: {path}", file=sys.stderr)
        return 1
    try:
        repo = json.loads(path.read_text())
    except json.JSONDecodeError as e:
        print(f"error: hit file is not valid JSON: {e}", file=sys.stderr)
        return 1
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="pheno-drift-detector",
        description=(
            "App-substrate drift detector — companion to pheno-predict "
            "and pheno-framework-lint (L74, ADR-049)."
        ),
    )
    ...
    scan.add_argument(
        "--format", choices=["json", "md", "gh-issues"], default="json"
    )
    ...
    score = sub.add_parser(
        "score",
        help="compute drift score for a single candidate repo path",
    )
    score.add_argument(
        "--candidate", required=True,
        help="path to a candidate repo (does NOT need to match an ADR-023 bucket name)",
    )
    score.add_argument(
    ...
    lb = sub.add_parser(
        "list-buckets",
        help="list known ADR-023 buckets (PAUSED/CONDITIONAL/CAPSTONE) and which repos fall into each",
    )
    lb.add_argument(
        "--root", default=None,
        help="optional: scan this root and annotate each bucket with discovered repos",
    )
    lb.set_defaults(func=cmd_list_buckets)

    val = sub.add_parser("validate", help="validate a single drift hit (HITL gate)")
