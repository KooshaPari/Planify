...
export const handleOrderChange = (
  ...
  const destinationBlockIndex = dropAtEndOfList
    ? blockIds.length
    : blockIds.findIndex((id) => id === droppedBlockId);
  ...
  if (destinationBlockIndex === 0)
    updatedSortOrder = (getBlockById(blockIds[0])?.sort_order ?? 0) - 1000;
  ...
  // update the sort order to the average of the two adjacent blocks if dropped in between
    ...
    const relativeDestinationSortingOrder =
      getBlockById(blockIds[destinationBlockIndex - 1])?.sort_order ?? 0;
